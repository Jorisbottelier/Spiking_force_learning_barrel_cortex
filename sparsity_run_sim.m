function run = sparsity_run_sim(N, N_th, N_train, N_test, N_total, Win, G, Q,...
    Winp, alpha, Pexc, sparsity, FORCE, makespikes, input_type, savefolder)
...
param_comb = all_comb(Win, G, Q, Winp, Pexc, sparsity);
...
parameters_in = struct('N', N, 'N_th' , N_th, 'N_train', N_train,...
    ...
    'alpha', alpha, 'sparsity', sparsity, 'FORCE', FORCE, 'makespikes', makespikes,...
...
for i = 1: size(param_comb, 1) % change to parfor during paramsweep
    ...
    scale_parameters.sparsity = param_comb(i, 6);
    
    % run the network
    run{i}.parameters_out = sparsity_LIF_training(parameters_in,...
